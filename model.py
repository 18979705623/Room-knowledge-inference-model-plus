...
class EncoderLSTM(nn.Module):
    ''' 编码导航指令，返回隐藏状态上下文(用于注意力机制)和解码器的初始状态。 '''
    ...
    def __init__(self, vocab_size, embedding_size, hidden_size, padding_idx,
        ...
        if self.use_glove:
            print('Using GloVe embedding LSTM')
    ...
    def forward(self, inputs, lengths, seq_mask):
        ''' 使输入词汇表索引为(batch, seq_len)。同时也需要一个用于动态批处理的长度列表。 '''
...
class SoftDotAttention(nn.Module):
    ...
    def forward(self, h, context, mask=None):
        ...
        # TODO: attn = attn / math.sqrt(self.dim) # 防止极端的softmax

        if mask is not None:
            # 在softmax之前的掩蔽，填充为一个很大的负数
...
class FeedforwardImageAttention(nn.Module):
    def __init__(self, context_size, hidden_size, image_feature_size=640):
...
class BottomUpImageAttention(nn.Module):
    def __init__(self, context_size, object_embedding_size,
                 ...
                 num_attributes, image_feature_size=640):
...
class VisualSoftDotAttention(nn.Module):
    ...
    def forward(self, h, k, mask=None, v=None):
        ...
        h: batch x h_dim 批量，隐藏层维度
        k: batch x v_num x v_dim批量，可导航个数，图像特征维度/隐藏层维度
        '''
...
class EmbeddingEncoder(nn.Module):
    def __init__(self, vocab_size, embedding_size, hidden_size, padding_idx,
        ...
        if self.use_glove:
            print('Using GloVe embedding Embedding')
            self.embedding.weight.data[...] = torch.from_numpy(glove)
            self.embedding.weight.requires_grad = False
...
class TransformerEncoder(nn.Module):
    def __init__(self, vocab_size, embedding_size, hidden_size, padding_idx,
        ...
        self.use_glove = glove is not None
        if self.use_glove:
            print('Using GloVe embedding Tranformer')
            self.embedding.weight.data[...] = torch.from_numpy(glove)
            self.embedding.weight.requires_grad = False
        encoder_layer = torch.nn.TransformerEncoderLayer(d_model=embedding_size, nhead=nhead,
                                                         dim_feedforward=ff, dropout=dropout_ratio)
...
class object_roomTransformer(nn.Module):
    def __init__(self, embedding_size, hidden_size, dropout_ratio,
                 feature_size=640 + 128, image_attention_layers=None,
                 ...
                 max_degree=10, short_cut=False, ff=2048, label_length=5, soft_room_label=True, room_relation_vec=True,
                 object_top_n=5, load_room_relation_weight=True, load_object_relation_weight=True):
        ...
        num_rooms = 31
        repeat = 128
        decoder_layer = nn.TransformerDecoderLayer(hidden_size, num_head, dim_feedforward=ff,
        ...
        self.object_gcn = GCN_pre_define(300, process_num=num_gcn, max_degree=max_degree, short_cut=short_cut,
                                         load_adj_weight=load_object_relation_weight)
        # print('object_gcn:',self.object_gcn,type(self.object_gcn))#<class 'KB.ConceptNet.GCN_pre_define'>
        ...
        # print('action_selector:',self.action_selector,type(self.action_selector)) #<class 'model.WhSoftDotAttention'>
    ...
    def encode_label(self, labels):
        "编码物体标签"
        embeddings = self.object_gcn.kg.embeddings  # 知识库中检索出的所有物体特征
        zero_tensor = try_cuda(torch.zeros(1, 300))
        embeddings = torch.cat([zero_tensor, embeddings], axis=0)
        # print('embeddings:', embeddings.shape)  # 13558,300
        ...
        if label_num < 5:
            ...
            labels = labels[:, :5]
            # print('labels:', labels, labels.shape)  # 432 ,5
            origin_shape = labels.shape
            labels = labels.reshape(-1)
            embeding_features = embeddings[labels]  # 得到指定物体的特征
            # print('embeding_features:', embeding_features, embeding_features.shape)  # 2400/2160/2640,300
    ...
    def get_label_sim(self, text_label_class, view_label_class, repeat=128):
        "计算当前看到的房间类型和目标房间类型的连通概率，使用先验知识self.room_class_relation"
        if self.concate_room:
            return torch.cat([text_label_class, view_label_class], axis=1)
        # print('torch.cat([text_label_class,view_label_class])',torch.cat([text_label_class,view_label_class],axis=1),torch.cat([text_label_class,view_label_class],axis=1).shape)
    ...
    def _build_current_input(self, u_t_prev, all_u_t, visual_context,
        ...
        text_room_class = self.text_room_classifier(text_room_atten)  # 基于文本的房间分类
        ...
        encoded_room_feature = self.encode_label(view_label_set)  # torch.Size([432, 5, 300]) 只取了前5個标签拿去预测房间类型，太多模型太大
        encoded_room_feature = encoded_room_feature.reshape(batch_size * action_count, -1)  # torch.Size([432, 1500])
        view_room_class = self.view_room_classifier(encoded_room_feature)  # 房间类型分类器，只取了前五个物体标签，准确率受限
        # print('view_room_class:',view_room_class)
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        room_labels_list = torch.tensor([]).to(device)
        text_hard_room_list = torch.tensor([]).to(device)
        text_room_list = torch.tensor([]).to(device)
        # self.hard_room_list = torch.tensor([]).to(device)
        for index, row in enumerate(view_room_class):
            room_labels_list = torch.cat([room_labels_list, torch.argmax(row).unsqueeze(0)], 0)
        room_labels_list = room_labels_list.reshape(batch_size, action_count)
        # room_ground_list = torch.tensor([]).to(device)
        for index, row in enumerate(text_room_class):
            text_room_list = torch.cat([text_room_list, torch.argmax(row).unsqueeze(0)], 0)
        # text_room_list = text_room_list.reshape(batch_size)
        # print('view_room_class:',view_room_class,view_room_class.shape)
        # for index, row in enumerate(text_hard_room_label):
        #     text_hard_room_list = torch.cat([text_hard_room_list, torch.argmax(row).unsqueeze(0)], 0)
        # text_hard_room_list = text_hard_room_list.reshape(batch_size)

        # print('room_list:', text_room_list==text_hard_room_list)
        # print('text_hard_room_list:',text_hard_room_list,text_hard_room_list.shape)

        view_hard_room_label = hard_softmax(view_room_class)
        # for index, row in enumerate(view_hard_room_label):
        #     self.hard_room_list = torch.cat([self.hard_room_list, torch.argmax(row).unsqueeze(0)], 0)
        # print('room_list:', self.room_list==self.hard_room_list)
        # print('view_hard_room_label:',view_hard_room_label,view_hard_room_label.shape)

        text_hard_room_label_ = text_hard_room_label[:, None, :].expand(-1, action_count, -1). \
            reshape(action_count * batch_size, -1)

        if self.soft_room_label:  # 使用软标签，使选择动作变得更灵活
            sim = self.get_label_sim(view_room_class, text_room_class[:, None, :].expand(-1, action_count, -1). \
                                     reshape(action_count * batch_size, -1))
            # print('use_soft_room_label_sim_hat:', sim, sim.shape)
        else:  # 硬标签，经过softmax变成一个1，其他30个都是零
            sim = self.get_label_sim(text_hard_room_label_, view_hard_room_label)
            sim = sim.reshape(batch_size, action_count, -1)
            # print('hard_sim_hat:', sim, sim.shape)

        # ragular部分
        g_v = all_u_t.view(-1, self.feature_size)  # b*ac, 640+128z
        g_v = self.visual_mlp(g_v).view(batch_size, action_count, -1)  # b,ac,1024
        attn_vision, _alpha_vision = self.visual_attention_layer(h_0, g_v, v=all_u_t)  # b,640+128 #b,seq,1
        alpha_vision = self.sm(_alpha_vision)
        concat_input = torch.cat([attn_vision, u_t_prev], axis=1)
        if not self.wo_instr_input:
            concat_input = torch.cat([text_atten, concat_input], axis=1)  # b,640+128+640+128
        if self.use_object:
        ...
        return _input_curent, ctx_pos, text_atten, sim.reshape(batch_size, action_count, -1), \
               ...
               alpha_atten_obj, alpha_atten_room, object_index, room_labels_list, text_room_list

    def _forward_train(self, u_t_prev, all_u_t, visual_context, input_history, h_0, ctx,
                       ctx_mask, object_label_set=None, view_label_set=None, gt_labels=None):
        _input_curent, ctx_pos, text_atten, \
        room_features, g_v, attn_vision, text_alpha, \
        alpha_vision, text_room_class, view_room_loss, alpha_atten_obj, alpha_atten_room, object_index, room_labels_list, text_room_list = self._build_current_input(
            u_t_prev, all_u_t,
        ...
        if self.use_room:
            if gt_labels is not None:
                ...
                view_labels_oh.scatter_(1, view_labels[:, None], 1)
    ...
    def _forward_search(self, u_t_prev, all_u_t, visual_context, input_history, history_length, h_0, ctx,
        ...
        alpha_atten_obj, alpha_atten_room, object_index, room_labels_list, text_room_list = self._build_current_input(
            u_t_prev,
            all_u_t,
            visual_context,
            h_0, ctx,
        ...
        c_1 = c_1.sum(axis=1) / try_cuda(history_length[:, None].expand(batch_size, self.hidden_size)).clamp_min(
            1).float()
        ...
        if self.use_room:
            ...
            if gt_labels is not None:
                text_labels = gt_labels[0]
                view_labels = gt_labels[1]
                # print('view_labels[:, None]:', view_labels[:, None], view_labels[:, None].shape)
                text_labels_oh = try_cuda(torch.zeros(text_room_class.shape))
                view_labels_oh = try_cuda(torch.zeros(view_room_loss.shape))
                text_labels_oh.scatter_(1, text_labels[:, None], 1)
                view_labels_oh.scatter_(1, view_labels[:, None], 1)
                # print('text_labels_oh :', text_labels_oh, text_labels_oh.shape)
                # print('view_labels_oh :', view_labels_oh, view_labels_oh.shape)
                # print(torch.equal(view_labels_oh.cpu(), torch.zeros(view_labels_oh.shape))) # fei全零
                # print(torch.equal(text_labels_oh.cpu(), torch.zeros(text_labels_oh.shape))) # 非全零
                sim = self.get_label_sim(view_labels_oh, text_labels_oh[:, None, :].expand(-1, action_count, -1). \
                                         reshape(action_count * batch_size, -1))
                sim = sim.reshape(batch_size, action_count, -1)
                # print(torch.equal(sim.cpu(), torch.zeros(sim.shape))) # 全零
                # torch.save(sim, 'data/test_save_tensor.pt') # 保存數據
                room_features = sim
            g_v = torch.cat([g_v, room_features], axis=2)
        alpha_action = self.action_prediction(text_atten, h_1, all_u_t, g_v)
        new_length = history_length + 1
        to_input = torch.cat([to_input, try_cuda(torch.zeros(batch_size, 1, self.hidden_size))], dim=1)
        return h_1, c_1, to_input, new_length, text_atten, attn_vision, text_alpha, alpha_action, alpha_vision, room_labels_list, text_room_list

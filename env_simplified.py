            ...
            with open(mpfs, "rt") as tsv_in_file:
                ...
                for item in reader:
                    ...
                    features = np.frombuffer(decode_base64(item['features']), dtype=np.float32).reshape(
                        (ImageFeatures.NUM_VIEWS, ImageFeatures.MEAN_POOLED_DIM))  # TODO by wlt
...
class EnvBatch():
    '''一个简单的包装一批MatterSim环境，
        使用离散视点和预训练的特征，
        使用邻接字典来代替MatterSim模拟器
    '''
...
class R2RBatch():
    ...
    def observe(self, world_states, beamed=False, include_teacher=True, instr_id=None):
        ...
        # print('observe_world_states:', world_states, len(world_states))  # 每一个scan所有的历史视点
        # print('self.batch.__len__():', self.batch.__len__()) #batch_size
        for i, (state, adj_loc_list) in enumerate(zip(world_states, adj_loc_lists)):
            # if i > self.batch_size: continue
            item = self.batch[i]
...
class REVERIEBatch(R2RBatch):
    ...
    def __init__(self, image_features_list, batch_size=100, seed=10, splits=['train'], tokenizer=None, beam_size=1,
        ...
        for item in load_datasets(splits, type_='REVERIE'):
            ...
            instructions = item['instructions']  # REVERIE 指令2-3种
            if instruction_limit:
                instructions = instructions[:instruction_limit]
            for j, instr in enumerate(instructions):
                self.scans.append(item['scan'])
        ...
        # print('batch_len:', self.data.__len__())
        # print('room_label',self.room_label)
        print('R2RBatch loaded with %d instructions, using splits: %s' % (len(self.data), ",".join(splits)))
        self.batch = self.data
        # print('self.batch:', len(self.batch))
        self.notTest = ('test' not in splits)

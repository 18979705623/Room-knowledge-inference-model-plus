...
SearchState = namedtuple("SearchState",
                         "flogit,flogp, world_state, observation, action, action_embedding, action_count, h_t,c_t,father")  # flat_index,
TransformerSearchState = namedtuple("TransformerSearchState",
                                    "flogit,flogp, world_state, observation, action, action_embedding, action_count, h_t,c_t,father, history_input, history_length")  # flat_index,
...
def backchain_inference_states(last_inference_state):
    ...
    return list(reversed(states)), list(reversed(observations)), list(reversed(actions))[1:], list(reversed(scores))[
                                                                                              1:], list(
        reversed(attentions))[1:]  # 不包括启动动作


def least_common_viewpoint_path(inf_state_a, inf_state_b):
    # 返回从A到X，然后从Y到B的推理状态，
    # 其中X和Y分别是A和B的最小公共视点，它们共享一个viewpointId
...
def batch_instructions_from_encoded(encoded_instructions, max_length, reverse=False, sort=False, tok=None, addEos=True):
    # encoded_instructions:令牌索引列表(不应该被填充，或包含BOS或EOS令牌)
    # 确保pad不能作为任何句子的开头
...
class BaseAgent(object):
    ...
    def rollout(self):
        ''' 返回属性列表，包含：instr_id:'xx', path:[(viewpointId, heading_rad, elevation_rad)]  '''
...
class RandomAgent(BaseAgent):
    ''' 智能体随机选择一个方向然后试着径直前进五个视点步骤，然后停止。 '''
...
class Seq2SeqAgent(BaseAgent):
    ...
    def __init__(self, env, results_path, encoder, decoder, episode_len=10, beam_size=1, reverse_instruction=True,
                 max_instruction_length=80, attn_only_verb=False):
        super(Seq2SeqAgent, self).__init__(env, results_path)
        self.prog_monitor = True
    ...
    def _action_variable(self, obs):
        ...
        for i, ob in enumerate(obs):
            ...
            action_embeddings[i, :num_a, :] = ob['action_embedding'] #动作嵌入
        # print('is_valid:', is_valid, is_valid.shape)
        # print('action_embeddings:', action_embeddings, action_embeddings.shape)
        return (
    ...
    def _proc_batch(self, obs, beamed=False):
        ...
        return batch_instructions_from_encoded(encoded_instructions, self.max_instruction_length,
                                               reverse=self.reverse_instruction, tok=tok)
    ...
    def _score_obs_actions_and_instructions(self, path_obs, path_actions, encoded_instructions):
        ...
        u_t_prev = self.decoder.u_begin.expand(batch_size, -1)  # 初始化动作
        ...
        for t in range(self.episode_len):
            ...
            f_t_list = self._feature_variables(obs)  # 智能体观察到的的图像特征
            all_u_t, is_valid, _ = self._action_variable(obs)
            assert len(f_t_list) == 1
    ...
    def _search_collect(self, batch_queue, wss, current_idx, ended):
        ...
        # print('cand_wss:',cand_wss,len(cand_wss))
        # print('cand_acs:',cand_acs,len(cand_wss))
    ...
    def train(self, optimizers, schedulers, n_iters, feedback='teacher', use_angle_distance_loss=False,
              use_angle_distance_reward=False):
        ''' 进行给定迭代次数的训练 '''
        self.not_search()
    ...
    def load(self, path, load_scorer=False, **kwargs):
        ...
        print(self.encoder.load_state_dict(torch.load(encoder_path, **kwargs)))
...
class TransformerAgent(Seq2SeqAgent):
    ...
    def __init__(self, env, results_path, encoder, decoder, episode_len=10, beam_size=1, reverse_instruction=True,
        ...
        super(TransformerAgent, self).__init__(env, results_path, encoder, decoder, episode_len=10, beam_size=1,
                                               reverse_instruction=True, max_instruction_length=80,
    ...
    def compute_label_for_action(self, obs):
        "得到每个可导航视图包含的物体标签集，用于预测这个视图的房间类型"
        all_list = []
        max_num_a = 0
        for i, ob in enumerate(obs):
            max_num_a = max(max_num_a, len(ob['adj_loc_list']))
    ...
    def get_room_type_for_view(self, obs):
        ...
        for ob in obs:
            ...
            for adj_loc in ob['adj_loc_list']:
                nextViewpointId = adj_loc['nextViewpointId']
    ...
    def calculate_acc_room(self, ground, view, mask):
        ...
        # print('ground:', ground)
        # print('view', view)
        room_classify_result = (ground == view)[mask == 1]
        # print('room_classify_result:', room_classify_result)
        for index, tensor in enumerate(room_classify_result):
            if tensor:
                glob.add_num()
                num += 1
        glob.all_num(len(room_classify_result))
        # print('CORRECT_NUM,TOTAL_NUM:', CORRECT_NUM, TOTAL_NUM)
        acc_room_type = num / len(room_classify_result)
        return acc_room_type

    def calculate_acc_room_text(self, ground, text):
        "计算基于文本的房间分类准确率，即目标房间"
        num = 0
        room_classify_result = (ground == text)
        for index, tensor in enumerate(room_classify_result):
            if tensor:
                num += 1
        acc_room_type = num / len(room_classify_result)
    ...
    def _rollout_with_loss(self):
        ...
        # 向前通过编码器，给出初始隐藏状态和解码器的存储单元
        # TODO 考虑不把这个输入到解码器，只是使用注意力
        ...
        for t in range(self.episode_len + 1):
            ...
            assert len(f_t_list) == 1, '只能使用MeanPooled的特征'

            prev_h_t = h_t
            room_ground_list = self.get_room_type_for_view(obs)  # torch.Size([432])
            ...
            gt_labels = (text_room_target, room_ground_list)
            h_t, c_t, history_input, t_ground, v_ground, alpha_t, logit, alpha_v, text_room_class, view_room_class, text_obj_atten, text_room_atten, object_index = \
                self.decoder(u_t_prev, all_u_t, f_t_list[0], history_input, h_t, ctx, seq_mask, object_label_set,
                             action_label_set, gt_labels)
            ...
            if view_room_class is not None:
                ...
                loss = torch.nn.functional.cross_entropy(view_room_class, room_ground_list.long(),
                                                         reduction='none')  #
            ...
            if self.use_angle_distance_loss:
                # print('use_angle_distance_loss') #no
            ...
            for i, ob in enumerate(obs):
                if not ended[i]:
                    ...
                    traj[i]['room_class_view'].append(
                        view_room_class[i].detach().cpu().tolist()[:int(action_count[i].item())])
    ...
    def _rollout_with_search(self):
        ...
        if self.distance_aware:
            batch_queue = [PriorityQueueWithDistance(distanceMap=self.env.distances, weight=self.distance_weight) for _
                           in range(batch_size)]
        else:
        ...
        # print('visit_graphs:',visit_graphs.__len__(),type(visit_graphs))
        ...
        for t in range(self.search_episode_len):
            ...
            # print('history_input:',history_input[47].shape,history_length)
            # print('last_world_states:',len(last_world_states))
            # print('acs:', acs)
            # print('last_obs:', len(last_obs))
            if t > 0:
                ...
                obs = self.env.observe(world_states)
                # print('NO.{}time observe:'.format(t), obs)
                # print(numpy.array(obs[0]['feature']).shape)  # [batch,36,768]
                for i in range(batch_size):
                    if (not ended[i] and
                        ...
                        visit_graphs[i].add_edge(last_obs[i]['viewpoint'],
                                                 obs[i]['viewpoint'])
                        # print('obs_{}_viewpoint'.format(i),obs[i]['viewpoint'])
            ...
            f_t_list = self._feature_variables(obs)
            all_u_t, is_valid, _ = self._action_variable(obs)
            # print('all_u_t:', all_u_t.shape, '', 'is_valid:', is_valid.shape)  # all_u_t: torch.Size([batch, 10(max_num_action), 768])  is_valid: torch.Size([batch, 10])
            # 1. local scorer
            action_label_set = self.compute_label_for_action(obs)
            # print('action_label_set:', action_label_set,action_label_set.shape)
            # print('action_label_set:',action_label_set,action_label_set.shape)
            object_label_set = self.compute_label_for_point([ob['labels'] for ob in obs])
            # print('object_label_set:',object_label_set,object_label_set.shape)
            ...
            action_number = all_u_t.shape[1]
            room_ground_list = self.get_room_type_for_view(obs)  # torch.Size([432])
            # view_labels_oh = try_cuda(torch.zeros([batch_size * action_number, 31]))
            # view_labels_oh.scatter_(1, room_ground_list[:, None], 1)
            # print(view_labels_oh, view_labels_oh.shape)
            # room_target = room_target.expand((-1, action_number))
            # print('room_target2:', room_target, room_target.shape)
            # room_target = room_target.reshape(-1)  # torch.Size([432])

            text_room_target = try_cuda(torch.LongTensor([ob['end_room_type'] for ob in obs]))
            gt_labels = (text_room_target, room_ground_list)
            h_t, c_t, history_input, history_length, t_ground, v_ground, alpha_t, logit, alpha_v, room_labels_list, text_room_list = self.decoder(
                u_t_prev, all_u_t, f_t_list[0], history_input, history_length, prev_h_t, ctx, seq_mask,
                object_label_set, action_label_set, gt_labels)

            # 1.5 计算基于视觉的房间分类准确率
            # room_ground_list = room_ground_list.reshape(batch_size, action_number)
            # result_view = self.calculate_acc_room(room_ground_list, room_labels_list, is_valid)
            # print('NO.{}time acc:'.format(t), result_view)

            # print('room_ground_list:', room_ground_list)
            # print('room_labels_list:', room_labels_list)

            # 1.6 计算基于指令文本的房间分类准确率
            # print('text_room_target:', text_room_target, 'text_room_list:', text_room_list)
            # result_text = self.calculate_acc_room_text(text_room_target, text_room_list)
            # print('NO.{}time acc:'.format(t), result_text)
            ...
            # print('ended', ended)
            # print('ac_lens', ac_lens)
            # print('last_logit', last_logit, last_logp.shape)
            # print('_logit', _logit, _logit.shape)
            for idx in range(batch_size):

                if ended[idx]: continue

                _len = ac_lens[idx]  # 该视点的邻接视点的个数
                # print('_len:',_len)
                ...
                if batch_queue[idx].size() == 0:
                    batch_queue[idx].lock()
                    ended[idx] = True
            # print('batch_queue:', batch_queue[0], len(batch_queue), batch_queue[0].size())
            # if ending_queue[idx].size() > 0:
            #    batch_queue[idx].lock()
            #    ended[idx] = True

        # 对于那些没有结束的，尽量去到分数最高的终点
        # print(np.size(ended) - np.count_nonzero(ended))

        # 强制每次搜索选择最好的结束队列
        # 当结束队列与扩展队列具有相同的启发式分数时，这是相当于在扩张队列中到达最佳终点;
        ...
        for idx in range(batch_size):
            ...
            if hasattr(self, 'reranker') and ending_queue[idx].size() > 1:
                ...
                while num_candidates > 0 and ending_queue[idx].size() > 0:
                    ...
                    inputs.append(
                        [len(item.world_states), item.flogit.sum, item.flogit.mean, item.flogp.sum, item.flogp.mean,
                         item.pm, item.speaker] * 4)
                    num_candidates -= 1
    ...
    def test(self, use_dropout=False, feedback='argmax', allow_cheat=False, beam_size=1, distance_aware=True,
             distance_weight=1):
        ''' 对当前环境中的每条指令评估一次 '''
        self.losses = []
        self.ce_losses = []

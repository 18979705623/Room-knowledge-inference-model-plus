...
MAX_INPUT_LENGTH = 80  # 最大指令文本输入长度
from torch.optim.lr_scheduler import LambdaLR
import json

max_episode_len = 10  # 每个情节的最大步骤数
glove_path = 'data/train_glove.npy'
action_embedding_size = 640 + 128  # 动作空间维度：视觉图像特征维度+sim维度
hidden_size = 512
dropout_ratio = 0.5
learning_rate = 0.0001
weight_decay = 0.0005
FEATURE_SIZE = 640 + 128
log_every = 100
save_every = 10000
...
def train(args, train_env, agent, optimizers, schedulers, n_iters, log_every=log_every, val_envs=None):
    ...
    for idx in range(0, n_iters, log_every):
        ...
        agent.train(optimizers, schedulers, interval, feedback=args.feedback_method,
                    use_angle_distance_loss=args.use_angle_distance_loss,
                    use_angle_distance_reward=args.use_angle_distance_reward)
        ...
        for env_name, (val_env, evaluator) in sorted(val_envs.items()):
            ...
            agent.results_path = '%s/%s_%s_iter_%d.json' % (
                ...
                env_name, iter)
            agent.not_search()
            agent.test(use_dropout=False, feedback='argmax', distance_aware=args.distance_aware,
...
def make_follower(args, vocab):
    ...
    decoder = Decoder(
        ...
        load_room_relation_weight=args.not_load_room_relation_weight,
        load_object_relation_weight=args.not_load_object_relation_weight,
        num_gcn=args.num_gcn, max_degree=args.max_degree, short_cut=args.short_cut,
        soft_room_label=args.soft_room_label,
...
def make_arg_parser():
    ...
    parser.add_argument("--feedback_method",
                        choices=["sample", "teacher", "sample1step", "sample2step", "sample3step", "teacher+sample",
                                 "recover"], default="sample")
    ...
    parser.add_argument('--en_nlayer', type=int, default=4)

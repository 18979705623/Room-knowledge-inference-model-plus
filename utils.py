...
def load_datasets(splits,type_='R2R'):
    data = []
    file_path = os.path.dirname(__file__)
...
class Tokenizer(object):
    ''' 对句子进行标记和编码的类。 '''
    SENTENCE_SPLIT_REGEX = re.compile(r'(\W+)') # 对任何非字母数字字符进行拆分
...
def run(arg_parser, entry_function):
    ...
    args.RESULT_DIR = os.path.join('experiments/',args.experiment_name,'results')
    args.SNAPSHOT_DIR = os.path.join('experiments/',args.experiment_name,'snapshots')
    args.PLOT_DIR = os.path.join('experiments/',args.experiment_name,'plots')
    args.SEARCH_DIR = os.path.join('experiments/',args.experiment_name,'search')
    args.traj_dir = os.path.join('experiments/',args.experiment_name,'traj')
    make_dirs([args.RESULT_DIR, args.SNAPSHOT_DIR, args.PLOT_DIR,args.SEARCH_DIR,args.traj_dir])
...
class PriorityQueueWithDistance(object):
    def __init__(self,distanceMap=None,nodeMap=None,weight=1):
        self.queue = []
        self.priority = []
        self.pri = []
    ...
    def pop(self,current_vp=None):
        if not self.islock:
            max_pri  = float('-inf')
            max_idx = -1
            max_li = -1
            for li,(pri,ix) in enumerate(self.pri):
                if current_vp is not None:
                    target_vp = self.queue[ix].world_state.viewpointId
                    target_scan = self.queue[ix].world_state.scanId
                    distance = self.distance[target_scan][current_vp][target_vp]
                    distance = distance**self.weight
                    if distance < 1:
                        distance = 1
                    current_pri = pri*distance if pri < 0 else pri/distance
                else:
                    current_pri = pri
                if current_pri > max_pri:
                    max_pri = current_pri
                    max_idx = ix
                    max_li = li
            max_state = self.queue[max_idx]
            max_pri = self.priority[max_idx]
            del self.pri[max_li]
            return max_idx,max_pri,max_state
        else:
            return 0, self.priority[0], self.queue[0]
    ...
    def push(self,state,priority):
        ...
        self.priority.append(priority)
        self.pri.append((priority,self.len))
        self.len = self.len+1
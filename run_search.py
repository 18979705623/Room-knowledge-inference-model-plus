...
def cache(args, agent, train_env, val_envs):
    ...
    for env_name, env in zip(cache_env_name, cache_env):
        # 如果环境名不是 'val_unseen': continue
        agent.env = env
        if agent.speaker: agent.speaker.env = env
        print("Generating candidates for", env_name)
        agent.cache_search_candidates()
        if not args.no_save:
            with open('cache_{}{}{}{}.json'.format(env_name, '_debug' if args.debug else '', args.max_episode_len,
                                                   args.early_stop), 'w') as outfile:
                json.dump(agent.cache_candidates, outfile, cls=NumpyEncoder)
            with open('search_{}{}{}{}.json'.format(env_name, '_debug' if args.debug else '', args.max_episode_len,
                                                    args.early_stop), 'w') as outfile:
...
def main(args):
    ...
    correct_num, total_num = glob.get_all()
    # print('room classify acc:', correct_num / total_num, 'correct_num:', correct_num, 'total_num:', total_num)
    # Load speaker
